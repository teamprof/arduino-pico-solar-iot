...
*/
/*
  modified by teamprof.net@gmail
*/
...
#ifndef __ARDUINO_UART_IMPLEMENTATION__EXTRA
#define __ARDUINO_UART_IMPLEMENTATION__EXTRA
...
namespace solariot
{
	...
	class SerialExtra : public UART
	{
	public:
		SerialExtra(int tx, int rx, int rts = -1, int cts = -1) : UART(tx, rx, rts, cts), onRx(nullptr)
		{
		}

		void onRxEvent(mbed::Callback<void(void)> onRx)
		{
			this->onRx = onRx;
		}

	protected:
		mbed::Callback<void(void)> onRx;
